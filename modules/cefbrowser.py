...
def py_catchtoken(value, js_callback):
    print("Token", value)
...
class CEFBrowser(Widget, FocusBehavior):
    ...
    def __init__(self, url="", OnLoadingStateChange_callback = None, client_handler_callbacks = None, pos_hint={}, *largs, **dargs):
        self.url = url
        ...
        self.client_handler_callbacks      = client_handler_callbacks
        self.OnLoadingStateChange_callback = OnLoadingStateChange_callback
        ...
        with self.canvas:
            ...
            self.__rect = Rectangle(
                pos=self.pos, size=self.size, texture=self._texture, pos_hint=self.pos_hint)
        ...
        client_handler.client_handler_callbacks      = self.client_handler_callbacks
        client_handler.OnLoadingStateChange_callback = self.OnLoadingStateChange_callback
    ...
    def on_url(self, instance, value):
        if self._browser and value and value != self._browser.GetUrl():
            ...
            # )
            self.change_url(self.url) #Navigate(self.url)
    ...
    def keyboard_position_simple(
        ...
        if hasattr(keyboard_widget, "docked") and not keyboard_widget.docked:
...
class ClientHandler:
    browser_widgets = {}
    pending_popups = {}
    OnLoadingStateChange_callback = None
    OnLoadingStateChange_js       = None
    client_handler_callbacks      = None
    ...
    def OnConsoleMessage(self, browser, level, message, source, line):  # noqa: N802
    ...
    def OnLoadingStateChange(  # noqa: N802
        ...
        # insert Javascript bindings
        if not self.OnLoadingStateChange_js:
            bindings = cefpython.JavascriptBindings()
            if self.client_handler_callbacks:
                for callback_name in self.client_handler_callbacks:
                    bindings.SetFunction( str(callback_name), self.client_handler_callbacks[callback_name]) 
            bw._browser.SetJavascriptBindings(bindings)
            self.OnLoadingStateChange_js = True

        bw.can_go_back = can_go_back
        bw.can_go_forward = can_go_forward
        if not is_loading:
            bw.js._inject()
            self.OnLoadingStateChange_callback(bw)  # out callback             
               

    def OnLoadStart(self, browser, frame):  # noqa: N802
        bw = self.browser_widgets[browser]
        bw.dispatch("on_load_start", frame)
        browser.SetZoomLevel(-1.5) # this works at this point
        bw.focus = False
        
        if bw:
            bw._browser.SendFocusEvent(True)
            bindings = cefpython.JavascriptBindings()
            bindings.SetFunction("__kivy__keyboard_update", bw._keyboard_update)
            bindings.SetFunction("__kivy__selection_update", bw._selection_bubble._update)
            bw._browser.SetJavascriptBindings(bindings)
            js_code = """
